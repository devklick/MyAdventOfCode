...
namespace MyAdventOfCode.Y2015
{
    public class D6
    {
        ...
        public D6(ITestOutputHelper output)
        {
            _output = output;
        }

        [Theory]
        [InlineData("turn on 0,0 through 999,999", Operation.TurnOn, 0,0, 999,999, 1000000,0)]
        [InlineData("toggle 0,0 through 999,0", Operation.Toggle, 0,0, 999,0, 1000, 999000)]
        [InlineData("turn off 499,499 through 500,500", Operation.TurnOff, 499,499, 500,500, 0, 1000000)]
        public void Part_1_Example(
            string instructionTemplate, Operation expectedOperation, int fromX, int fromY, int toX, int toY, int expectedOn, int expectedOff)
        {
            var grid = LightGrid.Default;
            var instruction = Instruction.Parse(instructionTemplate);

            // Verify the parsing of the instruction
            Assert.Equal(expectedOperation, instruction.Operation);
            Assert.Equal(fromX, instruction.Range.From.X);
            Assert.Equal(fromY, instruction.Range.From.Y);
            Assert.Equal(toX, instruction.Range.To.X);
        ...
        [Theory]
        [InlineData(3, 3, 3,
            "turn on 0,0 through 2,2", // turn all 9 on
            "turn off 0,0 through 2,0", // turn bottom 3 off
            "turn off 0,2 through 2,2", // turn top 3 off
            "turn off 0,0 through 0,2", // turn left 3 off
            "toggle 2,0 through 2,2")] // toggle right 3
        public void Part_1_CustomExample_MultipleInstructions(int gridX, int gridY, int expectedOn, params string[] instructionsTemplates)
        {
            var grid = new LightGrid(gridX, gridY);
        ...
        /// on one or more <see cref="Light"/>'s within a given <see cref="Range"/>
        /// </summary>
        private class Instruction
        {
            /// <summary>
            /// The type of operation to be performed on the light(s)
            /// </summary>
            ...
            /// Each instruction in the string should be seperated by a new line.
            /// See <see cref="Parse(string)"/> for format of each instruction.
            /// </summary>
            /// <param name="instructionTemplateList">The string containing multiple instructions to be parsed.</param>
            /// <returns>A collection of instructions.</returns>
            public static List<Instruction> ParseMultiple(string instructionTemplateList)
            {
                var instructions = new List<Instruction>();
                
                foreach(var i in instructionTemplateList.Split(Environment.NewLine))
                {
                    instructions.Add(Parse(i));
                }
            ...
            /// </summary>
            /// <param name="instructionTemplate"></param>
            /// <returns></returns>
            /// <remarks>
            /// The string must be in a specific format. 4 pieces of infomation are expected, separated by spaces:
            /// <list type="bullet">
            /// <item>Type of operation ("turn on", "turn off", "toggle")</item>
            /// <item>Start Position in X,Y format ("0,0")</item>
            /// <item>The word "through"</item>
            /// <item>End Position in X,Y format ("10,10")</item>
            /// </list>
            /// </remarks>
            /// <example>
            /// <list type="bullet">
            /// <item>turn on 0,0 through 999,999</item>
            /// <item>turn off 0,2 through 2,2</item>
            /// <item>toggle 1,50 through 5,50</item>
            /// </list>
            /// </example>
            public static Instruction Parse(string instructionTemplate)
            {
                var instruction = new Instruction();

                var instructionParts = instructionTemplate.ToLower().Split(" ");

                if (instructionParts[0] == "toggle")
                {
                    instruction.Operation = Operation.Toggle;
                    instruction.Range = GetRange(instructionParts, 0);
                }
                else if (instructionParts[0] == "turn")
                {
                    if (instructionParts[1] == "on")
                    {
                        instruction.Operation = Operation.TurnOn;
                    }
                    else if (instructionParts[1] == "off")
                    {
                        instruction.Operation = Operation.TurnOff;
                    }
                    else ThrowFormatException();

                    instruction.Range = GetRange(instructionParts, 1);
                }
                else ThrowFormatException();
            ...
            private static void ThrowFormatException(string message = "The input provided is not in the expected format.") 
                => throw new FormatException(message);

            private static Range GetRange(string[] instructionParts, int wordShift)
            {
                var fromIndex = 1 + wordShift;
                var toIndex = 3 + wordShift;

                if (fromIndex > instructionParts.Length
                    || toIndex > instructionParts.Length)
                {
                ...
                return new Range
                {
                    From = Position.Parse(instructionParts[fromIndex]),
                    To = Position.Parse(instructionParts[toIndex])
                };
            }
        }

        /// <summary>
        /// The type of operation to be performed on a <see cref="Light"/>
        /// </summary>
        ...
        /// <summary>
        /// An inclusive range of <see cref="Position"/>'s.
        /// </summary>
        ...
        /// <summary>
        /// A matrix of <see cref="Light"/>'s.
        /// </summary>
        private class LightGrid
        {
            /// <summary>
            /// Creates a new instance with exactly 1,000,000 lights; 1,000 wide by 1,000 tall.
            /// </summary>
            ...
            public LightGrid(int width, int height)
            {
                ...
                _lights = new Light[Width, Height];

                for (int x = 0; x < width; x++)
                {
                    for (int y = 0; y < height; y++)
                    {
                        _lights[x, y] = new Light(x, y);
                        NumberOfLightsOff++;
                    }
                }
            }
            ...
            /// <summary>
            /// Performs an operation against a range of lights.
            /// </summary>
        ...
        /// <summary>
        /// A light that exists at a specific position within a <see cref="LightGrid"/>.
        /// </summary>
        private class Light
        {
            /// <summary>
            ...
            /// <summary>
            /// Performs the relevant operation against the light switch.
            /// </summary>
            /// <param name="operation"></param>
            /// <returns></returns>
            public SwitchState OperateSwitch(Operation operation)
            {
                if (operation == Operation.TurnOff)
                    TurnOff();
        ...
        private struct Position
        {
            ...
            /// <summary>
            /// Parses a string into a position.
            /// </summary>
            /// <param name="positionTemplate">The string containing a position.</param>
            /// <returns></returns>
            /// <remarks>
            /// Expected in the format:
            /// <code>x,y</code>
            /// </remarks>
            public static Position Parse(string positionTemplate)
            {
                var parts = positionTemplate.Split(",");

                if (parts.Length != 2)
                    throw new FormatException();

                if (!int.TryParse(parts.First(), out var x))
                    throw new FormatException();

                if (!int.TryParse(parts.Last(), out var y))
                    throw new FormatException();
